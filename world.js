function World(){
    ...
    this.add = function(obj){
		var tile = this.pointToTile({x:obj.x|0,y:obj.y|0});
	...
	this.getCell = function(x,y,cbfn){
		...
		if(!cell){
			map.cells[key] = cell = {items:[],key:key,active:false};
			var mappixels = bgctx.createImageData(celldim,celldim);
			cell.terrain = terrain.generateMapTile(mappixels.data,x*celldim,y*celldim,celldim,celldim);//canv.width,canv.height);
    ...
    this.rebuildVisibleMap=function(_bgctx){
		...
		var topLeft = this.pointToTile({x:originX,y:originY});
		var bottomRight =this.pointToTile({x:originX+canv.width,y:originY+canv.height});
		var srx = -(originX % celldim);if(srx>0)srx-=celldim;
		var sry = -(originY % celldim);if(sry>0)sry-=celldim;
		...
		for(var x=topLeft.x,rx=srx;x<=bottomRight.x;x++,rx+=celldim){
			var cell = this.getCell(x,y,function(cell){
				...
				bgctx.drawImage(cell.bgImage,rx,ry);//putImageData(cell.pixels,rx,ry);
